...
def ConoscenzaLivelloUno(list_words_meme, text_caption, df, k):
  #k = iperparametro che mi dice quanti relazioni in conceptnet devo prendere per ogni parola
  #df = dataframe con le descrizioni
  ...
  for word in list_words_meme:
    ...
    for i in range(0, len(df)):
      if word == df['word'][i]:
        ok = True
        indice = i #indice dove si trova la descrizione della word
        break #l'hai trovata non c'è bisogno di continuare a cercare
    if ok == True: #se hai le descrizioni di questa word
      for j in range(0,k):
        if df['word'][indice+j] == word:
          descrizioni.append(df['description'][indice+j])
        else:
...
def ConoscenzaLivelloDue(list_words_meme_lvl_1, text_caption, df, k):
  ...
  for i in range(len(list_words_meme_lvl_1)):
    ...
    for j in range(lunghezza):
      ...
      for c in range(0, len(df)):
        if list_words_meme_lvl_1[i][j] == df['word'][c]:
          ok = True
          indice = c
          break
      if ok == True:
        for r in range(0,k):
          if df['word'][indice+r] == list_words_meme_lvl_1[i][j]:
            lista_descrizioni_lvl_2.append(df['description'][indice+r])
          else:
            lista_descrizioni_lvl_2.append('None')   
...
def GenerazioneGrafo(list_words_meme, list_words_meme_lvl_1, list_words_meme_lvl_2, list_words_caption, sinonimi_lvl_0):
  ...
  #MODO2 -> COLLEGO A a B, B a C, C a D, ...
