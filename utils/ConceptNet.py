...
def BestDescrizione(lista_descrizioni, text_caption, k):
  ...
  #Ritorno una lista con l'indice dei k elementi più simili
  lista_piu_simili = []
...
def SearchOnConceptNet(query, text_caption, k):
  ...
  lista_descrizioni = []
  for i in obj['edges']:
    #evitiamo il caso in cui la descrizione sia una traduzione perchè i meme sono in inglese
    #evitiamo il caso in cui ConceptNet non ritorni descrizioni
...
def ConoscenzaLivelloUno(list_words_meme, text_caption, k):
  ...
  #Per ogni sinonimo trovato devo splittare e ricercare su conceptnet
  #->matrix of words
  temp = []
...
def GenerazioneGrafo(list_words_meme, list_words_meme_lvl_1, list_words_meme_lvl_2, list_words_caption, sinonimi_lvl_0):
  ...
  # Collego le parole estratte dal meme
  ...
  # Collego ogni parola di livello 1 con le sue descrizioni di livello 2
  for i in range (len(list_words_meme_lvl_1)):
    for j in range(len(list_words_meme_lvl_1[i])):
...
def GenerazioneGrafoSoloLV1(list_words_meme, list_words_meme_lvl_1, list_words_caption, sinonimi_lvl_0):
  # Genero ora la matrice di adiacenza che mi serve per generare il grafo
